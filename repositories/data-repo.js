...
module.exports = {
  insertSensorDataSet: async (sensorData) => {
    ...
    try {
      console.info(`insert sensorDataSet => ${JSON.stringify(sensorData)}`);
  ...
  insertStateDataSet: async (stateData) => {
    ...
    try {
      ...
      if (count[0].cnt == 1) {
        console.info(`update stateDataSet => ${JSON.stringify(stateData)}`);
        ...
        console.info(`insert stateDataSet => ${JSON.stringify(stateData)}`);
